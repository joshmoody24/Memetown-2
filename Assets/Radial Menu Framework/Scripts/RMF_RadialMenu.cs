...
public class RMF_RadialMenu : MonoBehaviour {
    ...
    void Update() {
        ...
        if (!useGamepad){
            //uses mouse position instead of velocity rawAngle = Mathf.Atan2(Input.mousePosition.y - rt.position.y, Input.mousePosition.x - rt.position.x) * Mathf.Rad2Deg;
            //custom velocity based implementation for FPS
            ...
            rawAngle = Mathf.Atan2(_smoothMouse.y, _smoothMouse.x) * Mathf.Rad2Deg;
        }
        else
        ...
        if (angleOffset != 0 && useLazySelection) {
            ...
            if (elements[index] != null) {
                ...
                if (Input.GetButtonUp("Fire2")){ //|| Input.GetButtonDown("Submit")) {
